...
class HandleCollisionsAction(Action):
    ...
    def _grow(self, cast):
        '''Makes the Snakes Grow as time passes by.
        
        Args: 

            cast (Cast): The cast of Actors in the game.
        '''
        if self._counter % 7 == 0: # because the conter is rasing it is going to grow the tail of each snake every time the conter can be exatly divided by 7
            snake1 = cast.get_first_actor("snakes")
            snake2 = cast.get_second_actor("snakes")
            snake1.grow_tail(1)
            snake2.grow_tail(1)
    ...
    def _handle_game_over(self, cast):
        """Shows the 'game over' message, calls the winner and turns the loser snake white if the game is over.
